<div class="topbar" id="topbar">
    <div class="d-flex align-items-center">
        ...
        <h4 class="mb-0 ms-3 fw-bold">@yield('pageTitle', '')</h4>
    </div>
    ...
    <div class="d-flex align-items-center">
        ...
        <div class="user-menu dropdown">
            <a
                ...
                aria-expanded="false">
                @auth
                    <img src="{{ session('user.photo', default_photo()) }}" alt="Profil" class="avatar" />
                    <div class="d-none d-md-block ms-2">
                        <span class="fw-semibold">{{ session('user.nom_complet', Auth::user()->personne->nom_complet ?? 'Utilisateur') }}</span>
                    </div>
                @else
                    <img src="{{ default_photo() }}" alt="Guest" class="avatar" />
            ...
            <ul class="dropdown-menu dropdown-menu-end shadow-lg border-0" style="min-width: 220px;" aria-labelledby="dropdownUser">
                ...
                <li class="px-3 py-2 border-bottom">
                    <div class="d-flex align-items-center">
                        <img src="{{ session('user.photo', default_photo()) }}" alt="Profil" class="avatar-lg me-3" />
                        <div>
                            <span class="d-block fw-bold">{{ session('user.nom_complet', 'Utilisateur') }}</span>
                            <small class="text-muted">{{ session('user.email', Auth::user()->personne->email ?? '') }}</small>
                        </div>
                    </div>
                </li>
                @endauth

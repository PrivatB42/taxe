...
//         const videoClasses =
//             this.useVideoJs && this.hasVideoJs
...
//             } catch (error) {
//                 console.error(
//                     "Erreur lors de l'initialisation de Video.js:",
//                     error
//                 );
//                 reject(error);
//             }
...
//     previewVideo(file) {
//         // Remplacer FileReader.readAsDataURL par URL.createObjectURL
...
//         console.log("Vidéo prévisualisée avec Object URL:", this.titre);

//         // Nettoyer l'URL objet lors de la destruction
//         this.cleanupObjectUrls();
//     }

//     // Méthode pour nettoyer les URLs objets créées
//     cleanupObjectUrls() {
//         // Stocker l'URL objet précédente pour la nettoyer plus tard
//         if (this.previousObjectUrl) {
//             URL.revokeObjectURL(this.previousObjectUrl);
//         }
//         this.previousObjectUrl = this.lienVideo;
//     }
...
class VideoDropzone {
    ...
    cleanupVideoJsInstance() {
        if (this.videoJsInstance) {
            try {
                // Vérifier si l'instance existe et n'est pas déjà disposée
                if (this.videoJsInstance.el_ && 
                    this.videoJsInstance.el_.parentNode && 
                    typeof this.videoJsInstance.dispose === 'function') {
                    this.videoJsInstance.dispose();
                }
            } catch (error) {
                console.warn("Erreur lors du nettoyage de Video.js:", error);
    ...
    renderVideoElement(videoId) {
        // NE PAS utiliser data-setup pour éviter la double initialisation
        const videoClasses =
            this.useVideoJs && this.hasVideoJs
                ? "video-js vjs-default-skin w-100 h-100"
                : "w-100 h-100";
    ...
    async initializeVideoJs() {
        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                // Initialiser Video.js manuellement sans data-setup
                this.videoJsInstance = window.videojs(videoElement, vjsConfig, function() {
                    console.log("Video.js est prêt et initialisé");
                });
...
class VideoDropzoneEnhanced extends VideoDropzone {
    ...
    initAnalytics() {
        ...
        const saved = localStorage.getItem(
            `video-analytics-${this.containerId}`
        );
        if (saved) {
    ...
    seekRelative(seconds) {
        if (this.videoJsInstance) {
            ...
            this.videoJsInstance.currentTime(
    ...
    adjustVolume(delta) {
        if (this.videoJsInstance) {
            const volume = Math.max(
                0,
                Math.min(1, this.videoJsInstance.volume() + delta)
            );
    ...
    showVolumeIndicator(volume) {
        ...
        indicator.className =
            "position-fixed top-50 start-50 translate-middle bg-dark bg-opacity-75 text-white p-3 rounded";
    ...
    isValidVideoFile(file) {
        ...
        if (!this.supportedFormats.includes(extension)) {
            this.showAlert(
                `Format non supporté. Formats acceptés: ${this.supportedFormats
                    .join(", ")
                    .toUpperCase()}`,
                "danger"
            );
        ...
        if (file.size > this.maxFileSize) {
            ...
            this.showAlert(
                `Fichier trop volumineux (${fileSizeMB}MB). Taille maximum: ${maxSizeMB}MB`,
                "danger"
            );
            return false;
        }
    ...
    renderSubmitButton() {
        ...
        return `
            <div class="text-center mt-3">
                <button class="btn btn-${
                    this.theme
                } btn-lg submit-btn shadow-sm px-4" 
    ...
    //             // Analytics
    //             if (this.enableAnalytics) {
    //                 this.analytics.uploadsCount =
    //                     (this.analytics.uploadsCount || 0) + 1;
    //                 this.saveAnalytics();
    //             }
    //         } else {
    ...
    xhr.upload.addEventListener("progress", (event) => {
        if (event.lengthComputable) {
            ...
            this.updateProgressBar();
            if (this.onUploadProgress) {
                this.onUploadProgress(this.uploadProgress, event);
            }
        }
    });
    ...
    xhr.onreadystatechange = () => {
        if (xhr.readyState === XMLHttpRequest.DONE) {
            ...
            if (xhr.status >= 200 && xhr.status < 300) {
                ...
                this.showAlert("Vidéo envoyée avec succès !", "success");
                if (result.videoUrl) this.updateVideo(result.videoUrl, result.titre);
            } else {
                this.showAlert(`Erreur lors de l'envoi: ${xhr.statusText}`, "danger");
            }
    ...
    getVideoInfo() {
        ...
        return {
            ...
            volume: this.videoJsInstance
                ? this.videoJsInstance.volume()
                : video.volume,
            muted: this.videoJsInstance
                ? this.videoJsInstance.muted()
                : video.muted,
            paused: this.videoJsInstance
                ? this.videoJsInstance.paused()
                : video.paused,
    ...
    dispose() {
        ...
        const styleEl = document.getElementById(
            `video-dropzone-custom-${this.containerId}`
        );
        if (styleEl) {
            styleEl.remove();
        }

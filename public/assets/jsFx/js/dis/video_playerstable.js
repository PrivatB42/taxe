class VideoDropzone {
    constructor(options = {}) {
        ...
        this.playbackRates = options.playbackRates || [0.5, 1, 1.25, 1.5, 2];
    ...
    renderVideoElement(videoId) {
        ...
        const videoAttributes =
            this.useVideoJs && this.hasVideoJs
                ? `data-setup='{"fluid": ${
                      this.fluid
                  }, "playbackRates": ${JSON.stringify(this.playbackRates)}}'`
                : "controls";

        return `
                    <video 
                        id="${videoId}"
                        class="w-100 h-100 object-fit-cover position-absolute top-0 start-0 d-none video-player ${videoClasses}" 
                        ${videoAttributes}
                        preload="metadata"
                        ${this.muted ? "muted" : ""}
    ...
    async initializeVideoJs() {
        ...
        return new Promise((resolve, reject) => {
            try {
                this.videoJsInstance = window.videojs(videoElement, {
                    fluid: this.fluid,
                    responsive: true,
    ...
    handleDrop(e) {
        ...
        const dropzone = e.target.closest(".dropzone-empty");
        if (dropzone) dropzone.style.transform = "scale(1)";
...
// Utilisation en mode lecture seule
const videoPlayer2 = createVideoDropzone({
    containerId: 'video-container-2',
    lienVideo: 'https://example.com/video2.mp4',
    mode: 'lecture',
...
// Utilisation pour upload seulement
...
class VideoDropzoneEnhanced extends VideoDropzone {
    constructor(options = {}) {
        ...
        // Nouvelles options
        this.maxFileSize = options.maxFileSize || 100 * 1024 * 1024; // 100MB par défaut
        ...
        return `
            <div class="text-center mt-3">
                ...
                <div class="mt-2">
                    <small class="text-muted">
                        <i class="fas fa-info-circle me-1"></i>
                        Taille maximale: ${Math.round(this.maxFileSize / (1024 * 1024))}MB
                        ${this.enableKeyboard ? ' • Raccourcis: Espace (play/pause), ← → (navigation), ↑ ↓ (volume), F (plein écran)' : ''}
                    </small>
                </div>
            </div>
        `;
